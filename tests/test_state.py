...
class TestState:
    ...
    def test_set_transition_adds_and_overwrites(self, initial_state, mock_dependencies):
        ...
        initial_state.set_transition("event1", target2)  # overwrite
    ...
    # Arrange
        ...
        state._graphics.loop = False            # הוסף את זה
        state._graphics.is_finished.return_value = False  # הוסף את זה כדי למנוע יציאה לקטע finished_rest
    ...
    def test_process_command_with_known_transition_resets_and_returns_target_state(self, initial_state, mock_command_idle):
        ...
        # Mock physics to provide get_pos and get_cell values
        state._physics.get_pos.return_value = (10, 20)
        state._physics.get_cell.return_value = (5, 5)

        # Mock reset method on target_state to track call
        ...
        # Check that the position and start_cell were updated on target's physics
